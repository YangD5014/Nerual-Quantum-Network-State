...
log_path = "./日志/nes_vmc_0829_K4_LiH_STO-3G_min120_gauge_reset.log"
...
SAVE_INTERVAL = 20         # 每多少步保存/追加一次 h5py
HISTORY_FILE = "./data/history_natural_gradient_LiH_molecule_K4.h5"
...
# ====================== 初始化/加载 h5py 历史文件 ======================
# 先尝试只读模式探测已有数据（swmr=True 避免写锁冲突）
first_step = None
last_step = None
if os.path.exists(HISTORY_FILE):
    try:
        with h5py.File(HISTORY_FILE, "r", swmr=True) as f:
            if "steps" in f and len(f["steps"]) > 0:
                steps_arr = f["steps"][:]
                first_step = int(steps_arr[0])
                last_step = int(steps_arr[-1])
                logger.info(f"检测到已有历史文件，上次保存 step={last_step}，将从 step={last_step + 1} 继续")
            else:
                logger.info("检测到历史文件但为空，将从头开始")
    except Exception as e:
        logger.warning(f"读取历史文件失败（可能被锁），将覆盖写入: {e}")

# 以追加模式打开（swmr=True 允许多个读者不阻塞写者）
h5_file = h5py.File(HISTORY_FILE, "a", swmr=True)
_datasets_created = False
if "steps" not in h5_file:
    _datasets_created = True
    h5_file.create_dataset("steps", shape=(0,), maxshape=(None,), dtype="i")
    h5_file.create_dataset("logpsi_mean", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.create_dataset("logpsi_min", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.create_dataset("logpsi_max", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.create_dataset("grad_norm_raw", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.create_dataset("grad_norm_natural", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.create_dataset("E_L_real", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.create_dataset("E_L_imag", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.create_dataset("loss", shape=(0,), maxshape=(None,), dtype="f8")
    h5_file.attrs["first_step"] = first_step if first_step is not None else 0
    h5_file.attrs["save_interval"] = SAVE_INTERVAL
h5_file.flush()
# ====================== 初始化结束 ======================
...
for step in range(N_ITER):
    ...
    # 每 SAVE_INTERVAL 步保存/追加一次 h5py
    if (step + 1) % SAVE_INTERVAL == 0:
        arr = jnp.array(steps_history, dtype="i")
        h5_file["steps"].resize(len(arr), axis=0); h5_file["steps"][:] = arr
        arr = jnp.array(logpsi_mean_history, dtype="f8")
        h5_file["logpsi_mean"].resize(len(arr), axis=0); h5_file["logpsi_mean"][:] = arr
        arr = jnp.array(logpsi_min_history, dtype="f8")
        h5_file["logpsi_min"].resize(len(arr), axis=0); h5_file["logpsi_min"][:] = arr
        arr = jnp.array(logpsi_max_history, dtype="f8")
        h5_file["logpsi_max"].resize(len(arr), axis=0); h5_file["logpsi_max"][:] = arr
        arr = jnp.array(grad_norm_raw_history, dtype="f8")
        h5_file["grad_norm_raw"].resize(len(arr), axis=0); h5_file["grad_norm_raw"][:] = arr
        arr = jnp.array(grad_norm_nat_history, dtype="f8")
        h5_file["grad_norm_natural"].resize(len(arr), axis=0); h5_file["grad_norm_natural"][:] = arr
        arr = jnp.array(E_L_real_history, dtype="f8")
        h5_file["E_L_real"].resize(len(arr), axis=0); h5_file["E_L_real"][:] = arr
        arr = jnp.array(E_L_imag_history, dtype="f8")
        h5_file["E_L_imag"].resize(len(arr), axis=0); h5_file["E_L_imag"][:] = arr
        arr = jnp.array(loss_history, dtype="f8")
        h5_file["loss"].resize(len(arr), axis=0); h5_file["loss"][:] = arr
        h5_file.flush()
        logger.info(f"[保存] Step {step} → h5py 文件已更新 ({len(steps_history)} 条记录)")
...
# 训练结束后做最后一次 flush 确保数据完整
h5_file["steps"] = jnp.array(steps_history, dtype="i")
h5_file["logpsi_mean"] = jnp.array(logpsi_mean_history, dtype="f8")
h5_file["logpsi_min"]  = jnp.array(logpsi_min_history, dtype="f8")
h5_file["logpsi_max"]  = jnp.array(logpsi_max_history, dtype="f8")
h5_file["grad_norm_raw"]      = jnp.array(grad_norm_raw_history, dtype="f8")
h5_file["grad_norm_natural"]  = jnp.array(grad_norm_nat_history, dtype="f8")
h5_file["E_L_real"] = jnp.array(E_L_real_history, dtype="f8")
h5_file["E_L_imag"] = jnp.array(E_L_imag_history, dtype="f8")
h5_file["loss"] = jnp.array(loss_history, dtype="f8")
h5_file.flush()
logger.info(f"[保存] 训练结束，h5py 最终写入 {len(steps_history)} 条记录 → {HISTORY_FILE}")
h5_file.close()
...
loss_fig_path = _save_curve(
    ...
    "./日志/loss_curve_gauge_reset_min120.png",
)
...
logpsi_fig_path = _save_curve(
    ...
    "./日志/logPsi_mean_curve_gauge_reset_min120.png",
)
