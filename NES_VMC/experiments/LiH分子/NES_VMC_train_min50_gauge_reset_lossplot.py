...
log_path = "./日志/nes_vmc_0829_K4_LiH_STO-3G_min50_gauge_reset_lossplot.log"
...
loss_history = []        # 每步 Loss（观测值，用于曲线图）
reset_boundaries = []    # 周期性 reset 的边界步（step 编号，用于图上竖线）
for step in range(N_ITER):
    ...
    loss_history.append(float(loss_mean))
    ...
    if (step + 1) % RESET_PERIOD == 0:
        ...
        reset_boundaries.append(step + 1)
        logger.info(
...
try:
    ...
    ax.plot(range(len(loss_history)), loss_history, marker="o", markersize=4,
            linewidth=1.2, color="tab:blue", label="Loss")
    for rb in reset_boundaries:
        ax.axvline(rb - 1, color="tab:red", linestyle="--", linewidth=1.2,
                   alpha=0.8)
        ax.text(rb - 1, ax.get_ylim()[0], f" reset@{rb}", color="tab:red",
                fontsize=9, va="bottom")
    ax.set_xlabel("Step")
    ax.set_ylabel("Loss")
    ax.set_title("P8 gauge_reset | LiH K=4 Loss vs Step (N_ITER=50, RESET_PERIOD=10)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    plot_path = "./日志/loss_curve_gauge_reset_50.png"
    fig.savefig(plot_path, dpi=150)
    plt.close(fig)
    logger.info(f"Loss 曲线图已保存: {plot_path}")
except Exception as e:
    logger.warning(f"绘制 Loss 曲线图失败: {e}")