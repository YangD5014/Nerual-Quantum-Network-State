...
# =============================================================================
# 4. 定义 TotalAnsatz（Flax 版本，支持批量输入）
# =============================================================================
...
class TotalAnsatzFlax(nn.Module):
    ...
    def __call__(self, x):
        """
        x: 扩展系统的组态
           - 单个样本: 形状 (K * n_spin_orbitals,)
           - 批量样本: 形状 (batch_size, K * n_spin_orbitals)
        返回: log|ψ|
        """
        # 处理输入形状
        if x.ndim == 1:
...
# 计算局域能量
def compute_local_energy_simple(params, ha_extended, x, eps=1e-12):
    """
    计算局部能量（简化版本）
    """
    # 获取连接组态和矩阵元
    x_conn, mels = ha_extended.get_conn_padded(x)
    
    # 计算当前组态的波函数值
    log_psi = forward_model(params, x)
    psi = jnp.exp(log_psi)
    
    # 计算所有连接组态的波函数值
    log_psi_conn = jax.vmap(lambda x_c: forward_model(params, x_c))(x_conn)
...
def compute_average_local_energy(params, ha_extended, samples):
    """计算平均局域能量"""
    def single_E_loc(x):
        return compute_local_energy_simple(params, ha_extended, x)
    
    E_locs = jax.vmap(single_E_loc)(samples)
    return E_locs.mean()


E_avg = compute_average_local_energy(params, ha_extended, samples.reshape(-1, hi_extended.size))
print(f"\n平均局部能量: {E_avg:.6f} Ha")
