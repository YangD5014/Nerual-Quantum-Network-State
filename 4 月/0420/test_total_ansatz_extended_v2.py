"""
测试使用 TotalAnsatz + 扩展哈密顿量训练（改进版）
使用原始哈密顿量的 get_conn_padded 方法
"""
import jax
...
print("=" * 60)
print("测试 TotalAnsatz + 扩展哈密顿量（改进版）")
print("=" * 60)
...
for step in tqdm(range(n_iter)):
    ...
    loss_val, grads = value_and_grad(
        (graph_def, params), ha_original, samples_flat
    )
...
E_avg, E_matrix_avg = compute_average_energy_matrix(
    total_ansatz, final_samples_flat, ha_original
)
