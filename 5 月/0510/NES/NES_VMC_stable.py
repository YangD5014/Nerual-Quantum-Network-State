"""
NES-VMC (Natural Excited State Variational Monte Carlo) 算法实现 - 数值稳定版
...
class NESTotalAnsatz(nnx.Module):
    ...
    def __call__(self, x: jax.Array):
        def _forward_single(x_single):
            ...
            Psi_matrix = jnp.exp(jnp.real(L)) * jnp.exp(1j * jnp.imag(L))
...
# 6. 损失函数和梯度
# ==============================================================================
def NES_loss_energy(ha, graphdef, params, x, reg=1e-6):
    """
    ...
    Psi_Matrix = Psi_Matrix + reg * jnp.eye(Psi_Matrix.shape[0])

    H_Psi = Ham_Psi(ha, total_model, x)

    # 使用更稳定的求解方式
    # E_L = Ψ^{-1} H_Psi = solve(Ψ, H_Psi)
    E_L = jnp.linalg.solve(Psi_Matrix, H_Psi)

    trace = jnp.trace(E_L)
    return jnp.real(trace), E_L

@partial(jax.vmap, in_axes=(None, None, None, 0, None))
def compute_local_energy_matrix_batch(ha, graphdef, params, x_batch, reg):
    loss_val, E_L = NES_loss_energy(ha, graphdef, params, x_batch, reg)
    return E_L

def nes_vmc_gradient(ha, graphdef, params, x_batch, machine, vmap_grad_logPsi, reg=1e-6):
    """计算 NES-VMC 梯度"""
    E_L_batch = compute_local_energy_matrix_batch(ha, graphdef, params, x_batch, reg)
...
if __name__ == '__main__':
    ...
    grad_logPsi = jax.grad(lambda p, s: jnp.real(machine(p, s)), argnums=0, holomorphic=False)
    ...
    for step in range(N_ITER):
        ...
        if grad_norm > 10.0:
            print(f"  警告：梯度范数过大 ({grad_norm:.2f})，进行裁剪")
            grad = jax.tree.map(lambda g: g * 10.0 / grad_norm, grad)
        ...
        try:
            ...
            # 检查自然梯度范数
            nat_grad_norm = jnp.linalg.norm(natural_grad)
            if nat_grad_norm > 10.0:
                print(f"  警告：自然梯度范数过大 ({nat_grad_norm:.2f})，进行裁剪")
                natural_grad = natural_grad * 10.0 / nat_grad_norm
    ...
    print(f"\n理论: E0 + E1 = {E_fcis[0] + E_fcis[1]:.8f} Ha")
    print(f"实际: Loss = {history['energy'][-1]:.8f} Ha")
