...
class NESFermionHopRule(nk.sampler.rules.MetropolisRule):
    edges: jnp.ndarray  # 跃迁边（改为jax数组，而非tuple）
    ...
    def transition(
        ...
        # 1. 随机选择跃迁边（修复：edges已是jax数组，可直接用jax索引）
        n_edges = self.edges.shape[0]  # 改为jax数组的shape获取长度
        edge_idx = jax.random.randint(key_edge, (n_chains,), 0, n_edges)
        selected_edges = self.edges[edge_idx]  # (n_chains, 2) 现在可正常索引
...
if __name__ == '__main__':
    # 修复1：构建扩展希尔伯特空间的跃迁边（基于单系统边 + 副本数K）
    # 单系统边：(0,1),(2,3) → 扩展后（K=2）：(0,1),(2,3),(4,5),(6,7)
    ext_edges = []
    for k in range(K):
    ...
    # 修复2：初始化rule时传入jax数组格式的edges
    rule = NESFermionHopRule(edges=ext_edges)
    
    # 测试1：校验所有态的重复子组态（正常）
    rule._check_duplicate_substate(hi_ext.all_states())
    
    # 测试2：调用transition（修复后可正常执行）
    test_sigma = jnp.array([[1,0,1,0,0,1,1,0]])  # (1,8) 单链测试
    new_sigma, _ = rule.transition(
        sampler=sampler,
        machine=machine,
        parameters=params,
        state=sampler_state,
    ...
    print(f"Transition result: {new_sigma}")