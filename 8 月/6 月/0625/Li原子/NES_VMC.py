...
def create_machine_gauge_stable(model: NESTotalAnsatz_gauge_stable):
    """
    采样器 / QGT / grad_logPsi 专用。

    假设 NESTotalAnsatz_gauge_stable 返回：
        log_Psi_gauge, L_stable, shift, L_gauge

    其中 log_Psi_gauge 已经是 gauge-fixed 后的真实 log det Ψ。
    """
    graphdef, state = nnx.split(model)
...
def create_machine_matrix_gauge_stable(model: NESTotalAnsatz_gauge_stable):
    """
...
def create_machine_max_gauge_stable(model: NESTotalAnsatz_gauge_stable):
    """
