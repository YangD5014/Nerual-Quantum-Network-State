...
class NESVMCSystem:
    ...
    def compute_hamiltonian_matrix(self):
        ...
        for i, state_i in enumerate(self.all_states):
            ...
            for j, x_prime in enumerate(x_primes):
                for k, state_k in enumerate(self.all_states):
                    if np.allclose(x_prime, state_k):
                        H_matrix[i, k] = mels[j]
                        break
        
        return H_matrix
    
    def compute_wavefunction_vector(self, model: SingleStateAnsatz) -> np.array:
        """计算波函数向量 (完整希尔伯特空间)"""
        psi = np.zeros(self.n_configs, dtype=complex)
        for i, state in enumerate(self.all_states):
            psi[i] = complex(model(state))
        return psi
    
    def compute_energy_exact(self, model: SingleStateAnsatz) -> float:
        """精确计算能量期望值 (使用完整希尔伯特空间)"""
        psi = self.compute_wavefunction_vector(model)
        H = self.compute_hamiltonian_matrix()
        
        numerator = np.conj(psi) @ H @ psi
        denominator = np.conj(psi) @ psi
        
        return float(np.real(numerator / denominator))
    
    def compute_overlap_exact(self, model1: SingleStateAnsatz, model2: SingleStateAnsatz) -> complex:
        """精确计算重叠 (使用完整希尔伯特空间)"""
    ...
    def normalize_model(self, model: SingleStateAnsatz) -> SingleStateAnsatz:
        """归一化波函数"""
        psi = self.compute_wavefunction_vector(model)
        norm = np.sqrt(np.abs(np.conj(psi) @ psi))
        
        if norm > 1e-10:
            model_state = nnx.split(model)
            graphdef, params = model_state
            
            def scale_fn(p, scale):
                if hasattr(p, 'dtype') and jnp.iscomplexobj(p):
                    return p / scale
                return p
            
            scaled_params = jax.tree_util.tree_map(lambda p: scale_fn(p, np.sqrt(norm)), params)
    ...
    def orthogonalize_model(self, model: SingleStateAnsatz, prev_models: list, 
        ...
        for step in range(n_steps):
            overlaps = [self.compute_overlap_exact(model, pm) for pm in prev_models]
            total_overlap_sq = sum(abs(o)**2 for o in overlaps)
        ...
        overlaps = [abs(self.compute_overlap_exact(model, pm)) for pm in prev_models]
    ...
    def train_single_state(self, model: SingleStateAnsatz, prev_models: list,
        ...
        for step in pbar:
            def loss_fn(state):
                ...
                for pm in prev_models:
                    overlap = self.compute_overlap_exact(model_obj, pm)
                    orth_loss = orth_loss + abs(overlap)**2
    ...
    def train(self, n_iterations: int = 1000, lr: float = 0.005, orth_weight: float = 100.0):
        ...
        for i in range(self.n_states):
            ...
            if i > 0:
                overlap = abs(self.compute_overlap_exact(self.models[i], self.models[0]))
    ...
    def _print_summary(self):
        ...
        for i in range(self.n_states):
            for j in range(i+1, self.n_states):
                overlap = abs(self.compute_overlap_exact(self.models[i], self.models[j]))
    ...
    def compute_excitation_via_hamiltonian_diagonalization(self):
        ...
        for i in range(self.n_states):
            for j in range(self.n_states):
                psi_i = self.compute_wavefunction_vector(self.models[i])
                psi_j = self.compute_wavefunction_vector(self.models[j])
                H = self.compute_hamiltonian_matrix()
