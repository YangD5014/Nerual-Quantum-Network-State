...
print("Optimized energy and relative error: ", mf_energy, error)# First we reset the parameters to run the optimisation again
vstate.init_parameters()
...
mf_energy = vstate.expect(H)
error = abs((mf_energy.mean - eig_vals[0]) / eig_vals[0])
print("Optimized energy and relative error: ", mf_energy, error)å

```
所以我期待你可以让我用上这样的官方封装好的 API 来完成我的NES-VMC 复现任务
