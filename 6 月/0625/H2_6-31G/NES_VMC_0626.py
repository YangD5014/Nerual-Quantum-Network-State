...
class SingleStateAnsatz(nnx.Module):
    """
    二次量子费米子单态对数振幅网络，NES-VMC 基础单元
    输入 x: (n_spin_orbitals,) jax.Array，元素 0/1，自旋轨道占据数
    输出: 复数标量 logψ(x)，无内置平移/中心化，规范修正交给外部wrapper
    """
    def __init__(
        self,
        n_spin_orbitals: int,
        ...
        *, rngs: nnx.Rngs
        ...
        # 三层复值FFNN，相比原两层提升多激发表达能力
        # 第一层
        key, k1 = jax.random.split(key)
        self.lin1 = nnx.Linear(
            self.n_spin, hidden_dim,
            param_dtype=complex, rngs=nnx.Rngs(k1)
        )
        # 第二层
        key, k2 = jax.random.split(key)
        self.lin2 = nnx.Linear(
            hidden_dim, hidden_dim,
            param_dtype=complex, rngs=nnx.Rngs(k2)
        )
        # 第三层
        key, k3 = jax.random.split(key)
        self.lin3 = nnx.Linear(
            hidden_dim, hidden_dim,
            param_dtype=complex, rngs=nnx.Rngs(k3)
        )
        # 输出层：映射到单复数log振幅
        key, k_out = jax.random.split(key)
        self.lin_out = nnx.Linear(
            hidden_dim, 1,
            param_dtype=complex, rngs=nnx.Rngs(k_out)
        )

        # 二次量子两体Jastrow关联项，专门描述双激发组态耦合（6-31G关键）
        key, k_jast = jax.random.split(key)
        self.jastrow_mat = nnx.Linear(
            self.n_spin, self.n_spin,
            param_dtype=complex, rngs=nnx.Rngs(k_jast)
        )

    def __call__(self, x: jax.Array) -> jax.Array:
        # 兼容两种输入：
        # 1. 单构型 x: (n_spin,)  一维
        # 2. 哈密顿连接批量 x: (n_conn, n_spin) 二维
        x_c = x.astype(jnp.complex64)

        # 三层FFNN前向
        h = nnx.tanh(self.lin1(x_c))
        h = nnx.tanh(self.lin2(h))
        h = nnx.tanh(self.lin3(h))
        log_ffnn = jnp.squeeze(self.lin_out(h), axis=-1)

        # Jastrow两体项：用sum替代einsum，彻底解决下标维度报错
        Wx = self.jastrow_mat(x_c)
        log_jast = 0.5 * jnp.sum(x_c * Wx, axis=-1)

        log_psi_raw = log_ffnn + log_jast
        return log_psi_raw
...
def create_single_machine_gauge_fixed(single_model, ref_state):
    """
...
def create_gauge_fixed_total_machines(total_model, ref_state):
    """
    不修改 NESTotalAnsatz_stable 本体，
    在 wrapper 层构造 gauge-fixed L 矩阵。

    返回:
        total_machine(params, sigma)
        total_matrix_machine(params, sigma)
        total_max_machine(params, sigma)
        graphdef
        state
    """
    ...
    def _stable_from_L(L):
        ...
        返回:
            log_det_raw_centered, L_stable, shift
        """
        shift = jnp.max(jnp.real(L))
        shift = jax.lax.stop_gradient(shift)
        ...
        # 因为 det(exp(L - shift)) = exp(-K shift) det(exp(L))
        # 所以 centered raw total log amplitude:
    ...
    def total_machine(params, sigma):
        """
        给 sampler 用。
        返回 gauge-fixed total log amplitude。

        注意:
            这个和 raw total log amplitude 只差一个与 sigma 无关的参数常数，
            所以 Metropolis 采样分布不变。
        """
        m = nnx.merge(graphdef, params)

        sigma = jnp.asarray(sigma)

        # 单个样本: (K*n_spin,) 或 (K,n_spin)
        if sigma.ndim == 1 or sigma.ndim == 2:
            x_single = _normalize_x(sigma)
            L = _compute_L_centered_single(m, x_single)
            log_det_centered, _, _ = _stable_from_L(L)
            return log_det_centered

        # batch: (batch,K,n_spin)
        elif sigma.ndim == 3:
            def _one(x_single):
                L = _compute_L_centered_single(m, x_single)
                log_det_centered, _, _ = _stable_from_L(L)
    ...
    def total_matrix_machine(params, sigma):
        """
        给 loss / Ham_Psi_scaled 用。
        返回 L_stable，不是 raw L。
        """
        m = nnx.merge(graphdef, params)
